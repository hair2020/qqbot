...
__plugin_name__ = '歌手搜索'
__plugin_usage__ = '输入歌手歌手名'

bcc = get.bcc()

@bcc.receiver('GroupMessage',dispatchers=[
    Kanata([FullMatch("歌手"),RequireParam(name='music')])
])
async def geshou(
    ...
    try:
        ...
        for i in res['result']['songs'][:5]:
            ids = i['id']
            name = i['name']
            author = i['artists'][0]['name']
            bofang = f'https://music.163.com/song/media/outer/url?id={ids}.mp3'

            await app.sendGroupMessage(group,MessageChain.create([Plain(f"歌名:{name}\n歌手:{author}\n"+bofang)]))
    except Exception:
