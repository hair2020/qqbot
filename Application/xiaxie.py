...
async def report(
    ...
    if mes.startswith('瞎写'):
        if mes == '瞎写':
            await app.sendGroupMessage(group,MessageChain.create([Plain('你想写啥？真笨')]))
            return
        else:
            if re.findall('\d+',mes):
                if re.findall("瞎写(.*?)[0-9]", mes) != ['']:
                    xx = re.findall('瞎写(.*?)[0-9]',mes)[0]# 主题
                    zs = int(re.findall('\d+', mes)[-1])
                    tmp = str()
                    while (len(tmp) < zs):
                    ...
                    await app.sendGroupMessage(group, MessageChain.create([Plain(tmp)]))
                else:
                    await app.sendGroupMessage(group, MessageChain.create([Plain('你这主题没法写')]))
                    return
            else:
                await app.sendGroupMessage(group,MessageChain.create([Plain('字数呢？真笨')]))
                return
